...
class AvgAccumulatorTest(BaseTestCase):
    """Test case for $avg accumulator."""
...
AVG_NULL_MISSING_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
        "null_all_null",
        docs=[{"v": None}, {"v": None}],
        expression="$v",
...
AVG_NON_NUMERIC_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
        "non_numeric_string",
...
AVG_SPECIAL_NUMERIC_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
...
# precision loss.
AVG_INTEGER_BOUNDARY_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
    ...
    AvgAccumulatorTest(
        ...
        expression="$v",
        expected=DOUBLE_FROM_INT64_MAX / 2,
...
AVG_FLOAT_BOUNDARY_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
...
AVG_DECIMAL128_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
        ...
        expression="$v",
        expected=Decimal128("1.000000000000000000000000000000002"),
...
AVG_OVERFLOW_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
        "overflow_double_max",
...
AVG_EXPRESSION_ARGS_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
...
AVG_EDGE_CASE_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
        "edge_single_int32",
        docs=[{"v": 7}],
        expression="$v",
...
AVG_EXPRESSION_ERROR_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
...
def test_accumulator_avg(collection, test_case: AvgAccumulatorTest):
    ...
    result = execute_command(
        collection,
        {
            ...
            "pipeline": [
                {"$group": {"_id": None, "result": {"$avg": test_case.expression}}},
...
AVG_RETURN_TYPE_TESTS: list[AvgAccumulatorTest] = [
    AvgAccumulatorTest(
        "type_int32_only",
        docs=[{"v": 2}, {"v": 4}],
        expression="$v",
...
def test_accumulator_avg_return_type(collection, test_case: AvgAccumulatorTest):
    ...
    assertSuccess(result, [{"type": test_case.expected}], msg=test_case.msg)


@dataclass(frozen=True)
class AvgArityTest(BaseTestCase):
    """Test case for $avg arity rejection."""
...
AVG_ARITY_TESTS: list[AvgArityTest] = [
    AvgArityTest(
...
def test_accumulator_avg_arity(collection, test_case: AvgArityTest):
